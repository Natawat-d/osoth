...
export async function seedBase(db) {
  ...
  await db.collection("branches").insertMany([
    { branch_ID: "BR-001", name: "สาขาหลัก", address: "อาคารสยามพารากอน ชั้น 4 กรุงเทพฯ", phone: "02-000-0001", line_id: "@osoth-siam", storefront_enabled: true, active: true, ...ts },
  ]);

  // V2: ข้อมูลบริษัท (singleton) — จำเป็นสำหรับ about_me/setup/แบรนด์
  await db.collection("companies").insertOne({
    company_ID: "CO-001",
    name: "โอสถ คลินิกความงาม จำกัด",
    address: "อาคารสยามพารากอน ชั้น 4 กรุงเทพฯ",
    tax_id: "0105567890123", phone: "02-000-0001", email: "contact@osoth.local",
    brand: { display_name: "Osoth", logo: "/brand/logo.jpg", primary_color: "#1560a3", tagline: "คลินิกความงาม", about: "ดูแลผิวพรรณและความงามครบวงจร โดยทีมแพทย์ผู้เชี่ยวชาญ" },
    ...ts,
  });
  ...
  await db.collection("users").insertMany([
    ...
    { user_ID: "US-002", branch_ID: "BR-001", role: "admin", full_name: "แอดมิน หน้าร้าน", nick_name: "มิน", email: "admin@osoth.local", phone: "", commission_rate: 0, color: "#b3282d", salary: 18000, active: true, ...ts },
    { user_ID: "US-004", branch_ID: "BR-001", role: "sale", full_name: "เซลล์ คนเก่ง", nick_name: "เซล", email: "sale@osoth.local", phone: "", commission_rate: 5, color: "#1a4e8a", salary: 15000, active: true, ...ts },
    ...
    { user_ID: "US-007", branch_ID: "BR-001", role: "BT", full_name: "บิวตี้ เทอราปิสต์ หนึ่ง", nick_name: "บีที1", email: "bt1@osoth.local", phone: "", commission_rate: 0, color: "#555", salary: 14000, active: true, ...ts },
    { user_ID: "US-008", branch_ID: "BR-001", role: "BT", full_name: "บิวตี้ เทอราปิสต์ สอง", nick_name: "บีที2", email: "bt2@osoth.local", phone: "", commission_rate: 0, color: "#777", salary: 12000, active: true, ...ts },
  ]);
  ...
  // V3.1: ลงเวรเป็นช่วงวันที่ (วันนี้−7 ถึง วันนี้+21)
  const dstr = (n) => { const x = new Date(Date.now() + n * 86400000); return `${x.getFullYear()}-${String(x.getMonth() + 1).padStart(2, "0")}-${String(x.getDate()).padStart(2, "0")}`; };
  await db.collection("doctorschedules").insertMany([
    {
      branch_ID: "BR-001", doctor_ID: "US-005",
      assignments: [{ date_start: dstr(-7), date_end: dstr(21), time_start: "10:00", time_end: "18:00", room_ID: "RM-001", note: "" }],
      overrides: [], ...ts,
    },
    {
      branch_ID: "BR-001", doctor_ID: "US-006",
      assignments: [{ date_start: dstr(-7), date_end: dstr(21), time_start: "12:00", time_end: "20:00", room_ID: "RM-002", note: "" }],
      overrides: [], ...ts,
    },
  ]);
  ...
  const IMG = { "CS-001": "botox", "CS-002": "skincare", "CS-003": "promo", "CS-004": "filler", "CS-005": "thread", "CS-006": "skincare", "CS-007": "combo", "CS-008": "gold" };
...
async function main() {
  ...
  console.log("seed เสร็จ ✓ — V3 5 roles (owner/admin/sale/dr×2/bt×2) · catalog + commission + company");

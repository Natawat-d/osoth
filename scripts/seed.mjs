...
// ล้างและใส่ข้อมูลใหม่ทั้งหมด ใช้ตอนเริ่มระบบ/ทดสอบเท่านั้น
...
async function main() {
  ...
  await db.collection("medicalprocedures").insertMany([
    { medical_procedure_ID: "MP-001", name: "ฉีด Botox", type: "doctor", cost: 500, active: true, ...ts },
    { medical_procedure_ID: "MP-002", name: "เตรียมผิว/แปะยาชา", type: "BT", cost: 150, active: true, ...ts },
    { medical_procedure_ID: "MP-003", name: "ร้อยไหม", type: "doctor", cost: 800, active: true, ...ts },
    { medical_procedure_ID: "MP-004", name: "ทรีตเมนต์ผิว", type: "BT", cost: 200, active: true, ...ts },
  ]);

  await db.collection("products").insertMany([
    { product_ID: "PD-001", name: "Botox 100u (10cc)", type: "injection", unit: "ขวด", sub_unit: "cc", sub_unit_size: 10, default_uses_per_unit: 5, selling_price: 2500, reorder_point: 3, shelf_life_after_open_days: 30, active: true, ...ts },
    { product_ID: "PD-002", name: "Filler 1cc", type: "injection", unit: "กล่อง", sub_unit: "cc", sub_unit_size: 1, default_uses_per_unit: 1, selling_price: 8000, reorder_point: 2, shelf_life_after_open_days: 0, active: true, ...ts },
    { product_ID: "PD-003", name: "มาส์กหน้าทองคำ", type: "consumable", unit: "ชิ้น", sub_unit: "", sub_unit_size: 1, default_uses_per_unit: 1, selling_price: 900, reorder_point: 5, shelf_life_after_open_days: 0, active: true, ...ts },
  ]);

  await db.collection("courses").insertMany([
    {
      course_ID: "CS-001", name: "Botox หน้าเรียว 5 ครั้ง", quantity_used: 5, validity_days: 365, price: 15000,
    ...
    {
      course_ID: "CS-002", name: "ทรีตเมนต์ผิวใส 10 ครั้ง", quantity_used: 10, validity_days: 365, price: 8000,
    ...
    {
      course_ID: "CS-003", name: "[โปร] Botox 5+1 ครั้ง", quantity_used: 6, validity_days: 365, price: 15000,
      products: [{ product_ID: "PD-001", sub_unit_per_use: 2 }],
      BT_procedures: [{ medical_procedure_ID: "MP-002" }],
      doctor_procedures: [{ medical_procedure_ID: "MP-001" }],
  ...
  await db.collection("promotions").insertMany([
    { promotion_ID: "PM-001", name: "ลด 10% Botox หน้าเรียว", type: "discount", course_ID: "CS-001", discount_type: "percent", discount_value: 10, promo_course_ID: null, date_start: d(-30), date_end: d(60), banner_image: "", active: true, ...ts },
    { promotion_ID: "PM-002", name: "โปร Botox 5+1", type: "new_course", course_ID: null, discount_type: null, discount_value: 0, promo_course_ID: "CS-003", date_start: d(-10), date_end: d(30), banner_image: "", active: true, ...ts },
  ]);
  ...
  const items = [];
  let iv = 1;
  const mkItem = (product_ID, lot_ID, uses, cc) => ({
    item_ID: `IV-${String(iv++).padStart(6, "0")}`,
    branch_ID: "BR-001", product_ID, lot_ID, state: "unused",
    opened_at: null, open_expiry_at: null,
    uses_remaining: uses, cc_remaining: cc, usage_log: [], ...ts,
  });
  for (let i = 0; i < 3; i++) items.push(mkItem("PD-001", "LOT-00001", 5, 10));
  for (let i = 0; i < 3; i++) items.push(mkItem("PD-001", "LOT-00002", 5, 10));
  for (let i = 0; i < 10; i++) items.push(mkItem("PD-003", "LOT-00003", 1, 1));
  ...
  await db.collection("counters").insertMany([
    { key: "BR", seq: 10 }, { key: "RM", seq: 10 }, { key: "US", seq: 10 },
    { key: "MP", seq: 10 }, { key: "PD", seq: 10 }, { key: "CS", seq: 10 },
  ...
  console.log("seed เสร็จ ✓ — users 8 คน (mock login), courses 3, stock PD-001 6 ขวด");
